...
export default function WeeklyPlan ({weeklyPlan} : {weeklyPlan: WeeklyPlanType}) {
  ...
  return (
    <View as="div" maxWidth="80rem" margin={layout.direction === 'row' ? null : 'auto'}>
      <Flex {...layout} justifyItems="space-between">
        {
          days.map(
            d => {
              ...
              let color = 'primary'
              if (isPast) { color = 'secondary' }
              if (isToday) { color = 'alert' }
              return (
                <View as="div" shadow={isToday ? 'topmost' : null} padding={isToday ? 'small' : null} key={d}>
