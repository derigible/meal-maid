...
function ToggleButton({
  ...
  return (
    <View
      ...
      padding="x-small"
    >
      <Tooltip color="default" renderTip={`Use ${type} Layout`} on="hover">
        <Button
          icon={icon}
      ...
      <Flex>
        <Flex.Item>
          <ToggleButton
            type="Card"
            inverse={!isTable}
            toggle={() => setIsTable(false)}
            icon={IconLaunchLine}
          />
          <ToggleButton
            type="Table"
            inverse={isTable}
            toggle={() => setIsTable(true)}
            icon={IconTableLine}
          />
        </Flex.Item>
      </Flex>
      {isTable ? (
        <TableRecipes recipes={data.recipes} />
      ) : (
        <CardRecipes recipes={data.recipes} />
      )}
    </>
  );
}
