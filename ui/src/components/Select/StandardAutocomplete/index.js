...
import { IconSearchLine, IconUserSolid } from "@instructure/ui-icons";
...
export default class StandardAutocomplete extends React.Component<
  expectedProps,
  State
> {
  state = {
  ...
  handleOptionCreate = (e: any) => {
    if (keycode.isEventKey(e, "enter")) {
      const ignoreValues = this.props.ignoreOptionCreateValues
        ? this.props.ignoreOptionCreateValues
        : [];
      if (
        ...
        const cb = this.props.onOptionCreate
          ? this.props.onOptionCreate
          : _ => {};
        cb(e.target.value);
